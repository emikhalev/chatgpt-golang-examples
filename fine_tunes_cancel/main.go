...
func main() {
	...
	{ // otiai10
		client := otiai10.NewClient(apiKey)
		cancelTune, err := client.CancelFineTune(ctx, fineTuneID)
		fmt.Printf("otiai10 CancelFineTune: %v \n (err: %+v)\n\n",
			utils.SPrintStruct(cancelTune), err)
	}

	{ // sashabaranov
		client := sashabaranov.NewClient(apiKey)
		fineTune, err := client.CancelFineTune(ctx, fineTuneID)
		fmt.Printf("sashabaranov CancelFineTune: %v \n (err: %+v)\n\n",
